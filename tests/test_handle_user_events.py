...
def mock_prey_objects():
    ...
    for _ in range(PREY_MAX_COUNT):
        prey_object = Prey(-50, randint(200, 550), 154, 145, preyTilesetPath, 100)
...
@pytest.mark.parametrize(
    "bulletsMaxCount, bulletsCount, score, preyScore, preyDefeatCount, expected_result",
    [
...
def test_handle_user_events_win(mock_pygame_event_quit, mock_prey_objects, mock_cursor_object,
    bulletsMaxCount, bulletsCount, score, preyScore, preyDefeatCount, expected_result):
    handle_user_events(bulletsMaxCount, 20, bulletsCount, mock_prey_objects[0], mock_cursor_object,  score, preyScore, preyDefeatCount)
    
    assert len(preyDefeatCount) == PREY_MAX_COUNT and preyDefeatCount.count(True) >= WIN_PREY_COUNT or \
           len(preyDefeatCount) == PREY_MAX_COUNT and preyDefeatCount.count(True) < WIN_PREY_COUNT
