...
def mock_prey_objects():
    prey_objects = []
    preyTilesetPath = os.path.dirname(os.path.abspath(__file__))
...
def mock_cursor_object():
    cursor_path = os.path.join(current_directory, "assets", "cursor.png")
    CURSOR = pygame.image.load(cursor_path)
