...
class Prey(GameChar):
    ...
    Attributes:
        ...
        horizontalFlyRow (int): The row in the tileset for the horizontal
        flying animation.
        upFlyRow (int): The row in the tileset for the upward flying animation.
        moveFrameOrder (list): The order of the frames in the tileset for the
        flying animation.

    Methods:
        update(): Updates the prey's random-directioned position, pray's
        default movement while hunting.
        ...
        change_direction(): Changes the direction of the prey based on its
        velocity.
        move(): Moves the prey based on its velocity and changes velocity if
        it hits the screen bounds.
    """
    minVelocity = 4
    maxVelocity = 6
    ...
    def __init__(self, x, y, width, height, tileset, killPrice, direction=1):
        ...
        Args:
            ...
            direction (int): The direction the prey is facing. 1 for right, -1
            for left.
        """
    ...
    def update(self):
        """
        Updates the prey's random position and draws it, pray's default
    ...
    def draw(self):
        ...
        image = pygame.transform.scale(
            self.tileset[self.moveFrameOrder[self.frame]][self.direction],
            (self.width, self.height)
        )
        self.changeDirection()
        SCREEN.blit(
            pygame.transform.flip(image, self.flipX, False),
            (self.x, self.y)
        )
    ...
    def changeDirection(self):
        ...
        Contains logic for changing line of tileset and flipping by X-axis the
        image based on the velocity positive or negative value.
        """
        self.flipX = False
    ...
    def move(self):
        ...
        Contains logic for changing prey's direction and x,y coordinate of the
        GameObj if any coordinate of pray is less than screen bounds or
        greater than screen bounds
        """
        # Changing the value if speed_x and speed_y both are zero
        if self.velocity == [0, 0]:
            self.velocity = [
                randint(self.minVelocity, self.maxVelocity) * self.direction,
                randint(self.minVelocity, self.maxVelocity) * self.direction
            ]
        # Changing the direction and x,y coordinate of the GameObj if the
        # coordinate of left side is less..or right side coordinate is greater
        if self.x <= BOUNDS_X[0] or self.x + self.width >= BOUNDS_X[1]:
            self.moveDirection = (
                -1 if self.x + self.width >= BOUNDS_X[1] else 1
            )
            self.velocity[0] = (
                choice([0, randint(self.minVelocity, self.maxVelocity)]) *
                self.moveDirection
            )
            self.velocity[1] = (
                choice([0, randint(self.minVelocity, self.maxVelocity)]) *
        ...
        # Changing the direction and x,y coordinate of the GameObj if the
        # coordinate of top side is less..
        # or bottom side coordinate is greater..
        if self.y <= BOUNDS_Y[0] or self.y + self.height >= BOUNDS_Y[1]:
            self.moveDirection = (
                -1 if self.y + self.height >= BOUNDS_Y[1] else 1
            )
            self.velocity[0] = (
                choice([0, randint(self.minVelocity, self.maxVelocity)]) *
                self.moveDirection
    ...
    def dying(self):
        ...
        Prey has death animation 2 seconds, it's 2 cycles, and then goes down
        by value.
        """
        if self.frameTimer < CHARANIMATIONFPS*2:  # 2 cycles
            SCREEN.blit(
                pygame.transform.scale(self.tileset[0][2],
                                       (self.width, self.height)),
                (self.x, self.y)
            )
            self.frameTimer += 1
        ...
        self.y += 8
        SCREEN.blit(
            pygame.transform.scale(self.tileset[self.gooseFallChoice][2],
                                   (self.width, self.height)),
            (self.x, self.y)
        )
    ...
    def flyAway(self):
        ...
        Prey changes its position on screen diagonaly-up with given value to
        the left or right side depending on the side of the screen.
        """
        if self.x > WIDTH/2:
